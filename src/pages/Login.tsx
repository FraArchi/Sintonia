...
export default function Login() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(e.message || 'Errore durante il login. Controlla le credenziali.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const e = err as Error;
      setError(e.message || "Errore durante l'accesso con Google.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      >
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl border border-slate-200 p-8">
          <div className="text-center mb-8">
            <h1 className="text-2xl font-bold text-slate-900 mb-2">Bentornato</h1>
            <p className="text-slate-500">Accedi a Sintonia per continuare</p>
          </div>
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-semibold flex items-center justify-center gap-2 hover:shadow-lg hover:shadow-blue-500/30 transition-all disabled:opacity-50"
            >
